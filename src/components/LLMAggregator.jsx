    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/openai/ask`, {
      ...
      if(response.level.includes('product')) {
        if(response.level == 'compare_product') {
          ...
          setChatHistory((prevList) => [...prevList, {user_id: fingerprint, type: response.level, chat_id: chat_id, text: '', level: response.level, status_report: response.status_report, opinion: response.opinion, product: JSON.parse(response.answer)}]);
          setFilterList((prevList) => [...prevList, {user_id: fingerprint, type: response.level, chat_id: chat_id, text: '', level: response.level, status_report: response.status_report, opinion: response.opinion, product: JSON.parse(response.answer), verify_top_open: false, verify_bottom_open: false}]);
        }
      } else {
        setChatHistory((prevList) => [...prevList, {user_id: fingerprint, type: 'answer', chat_id: chat_id, text: response.answer, level: response.level, status_report: response.status_report, opinion: response.opinion, product: []}]);
        setFilterList((prevList) => [...prevList, {user_id: fingerprint, type: 'answer', chat_id: chat_id, text: response.answer, level: response.level, status_report: response.status_report, opinion: response.opinion, product: [], verify_top_open: false, verify_bottom_open: false}]);
      }
      if(!localStorage.getItem('chat_id')) {
        localStorage.setItem('chat_id', chat_id);
        setChatList((prevList) => [...prevList, {user_id: fingerprint, type: 'question', chat_id: chat_id, text: question}]);
      }
                ...
                <Dialog>
                  ...
                  <DialogContent
                    ...
                    <DialogFooter className="gap-20 flex !justify-center">
                      ...
                      <DialogClose>
                        <div className="bg-white text-black w-[60px] hover:bg-black hover:text-white text-center rounded-sm py-1 shadow-[2px_2px_2px_black] cursor-pointer" onClick={() => deleteChatHistory(item.chat_id, item.user_id)}>
                          yes
                        </div>
                      </DialogClose>
                    </DialogFooter>
          ...
          <div className="mx-auto flex flex-col flex-1 gap-4 md:gap-5 lg:gap-6 md:max-w-3xl xl:max-w-[48rem] justify-center items-center">
            ...
            <div className="flex w-full pb-2 cursor-text flex-col items-center justify-center rounded-[20px] border border-[#F1E2FA] contain-inline-size overflow-clip bg-white dark:bg-black shadow-[10px_1px_20px_1px_black]">
              ...
              <div className="flex w-full px-2">
                <img src="/image/logo.png" alt="logo" className="w-[30] ml-auto cursor-pointer" onClick={askQuestion}/>
              </div>
            </div>
          </div>
        </div>
